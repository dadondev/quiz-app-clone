...
function PasswordInput ({errors, touched, handleBlur, handleChange}:{errors:FormikErrors<{password:string}>, touched:FormikTouched<{password: string}>, handleChange:handleChange, handleBlur:handleBlur}){
    ...
    return <>
        <div className={"p-inputgroup grid grid-cols-[1fr_auto] gap-2"} >
            <InputText placeholder="Parol" type={type} name={"password"} className={"py-2"} onChange={handleChange} onBlur={handleBlur}/>
            <Button onClick={handleTypeChange} className={"!p-2 !px-3"} type={"button"}>
                {type === "password" ? <EyeIcon/> : <EyeSlashIcon/>}
            </Button>
        </div>
        <p data-open={errors.password && touched.password}
